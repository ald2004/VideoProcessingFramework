...
FFmpegDemuxer::FFmpegDemuxer(AVFormatContext *fmtcx) : fmtc(fmtcx) {
  ...
  is_mp4H264 = ((eVideoCodec == AV_CODEC_ID_H264||eVideoCodec == AV_CODEC_ID_MPEG4)  &&
		 (!strcmp(fmtc->iformat->long_name, "QuickTime / MOV") ||
                ...
                !strcmp(fmtc->iformat->long_name, "AVI (Audio Video Interleaved)"));
  ...
  if (!toAnnexB) {
    cout << "fmtc->iformat->long_name: " << fmtc->iformat->long_name << endl;
    cout << "eVideoCodec : " << (int)eVideoCodec << endl;
    cout << "AV_CODEC_ID_H264 is :" << (int)AV_CODEC_ID_H264 << endl;
    flush(cout);
